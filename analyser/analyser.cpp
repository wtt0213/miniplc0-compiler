...
namespace miniplc0 {
	...
	std::optional<CompilationError> Analyser::analyseMain() {
		...
		// <常量声明>
		auto err_1 = analyseConstantDeclaration();
		if(err_1.has_value())
			return err_1;

		// <变量声明>
		auto err_2 = analyseVariableDeclaration();
		if(err_2.has_value())
			return err_2;

		// <语句序列>
		auto err_3 = analyseStatementSequence();
		if(err_3.has_value())
			return err_3;
	...
	std::optional<CompilationError> Analyser::analyseConstantDeclaration() {
		...
		while (true) {
			...
			isConstant(next.value().GetValueString());
	...
	// <常表达式> ::= [<符号>]<无符号整数>
	// <符号> ::= '+' | '-'
	// <无符号整数> ::= tokenType::UnsignedInteger
	// 需要补全
	std::optional<CompilationError> Analyser::analyseConstantExpression(int32_t& out) {
		...
		// 注意以下均为常表达式
		// +1 -1 1
		// 同时要注意是否溢出::我这个词法分析进来的一定保证是合法的范围
		...
		if(next.value().GetType() == TokenType::PLUS_SIGN){
			...
			if(next2.value().GetType() != TokenType::UNSIGNED_INTEGER)
				return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrInvalidConstantValue);
	...
	std::optional<CompilationError> Analyser::analyseAssignmentStatement() { 
		...
		next = nextToken();
		if(!next.has_value()||next.value().GetType() != TokenType::EQUAL_SIGN)
			return std::make_optional<CompilationError>(_current_pos, ErrorCode::ErrNeedEqual);
		//<表达式>
		auto err = analyseExpression();
		if(err.has_value())
			return err;
	...
	bool Analyser::isInitializedVariable(const std::string&s) {
		return _initialized_vars.find(s) != _initialized_vars.end();
	}
